          ...
          <el-table-column align="center" label="操作" width="200px">
            <template slot-scope="scope">
              ...
              <el-popconfirm
                  ...
                  icon-color="red"
                  title="确认移除该教师？"
                  @confirm="pullTeacher(scope.row.username)"
              >
...
export default {
  ...
  data() {
    ...
    return {
      ...
      clazzId: this.$route.params.clazzId,
      title: [],
  ...
  methods: {
    query() {
      teacherApi.getCSPage(this.pageIndex,this.pageSize,"","",this.clazzId).then(res =>{
        this.total = res.result.total;
        this.tableData = res.result.data
      })
    },
    handlePageChange(pageIndex) {
      teacherApi.getCSPage(pageIndex,this.pageSize,this.queryForm.username,this.queryForm.name,this.clazzId).then(res => {
        this.total = res.result.total;
        this.tableData = res.result.data
      });
    },
    handleSizeChange(pageSize){
      teacherApi.getCSPageSize(pageSize,this.queryForm.username,this.queryForm.name,this.clazzId).then(res=>{
        this.total = res.result.total;
        this.tableData = res.result.data
      })
    },
    ...
    pullTeacher(username){
      teacherApi.pullTeacher(this.clazzId,username).then(res=>{
        if (res.code === 200){
