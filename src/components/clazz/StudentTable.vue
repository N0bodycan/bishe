        ...
        <el-form :model="entityForm" :rules="rules" ref="ruleForm" label-width="82px" class="demo-ruleForm">
          ...
          <el-form-item label="班级" prop="clazz">
            <el-select v-model="entityForm.clazz" placeholder="请选择班级">
              <el-option :label="clazzId" :value="clazzId"/>
            </el-select>
          </el-form-item>
...
export default {
  ...
  methods: {
    query() {
      studentmApi.getPage(this.pageIndex,this.pageSize,'','',this.clazzId).then(res =>{
    ...
    handlePageChange(pageIndex) {
      studentmApi.getPage(pageIndex,this.pageSize,this.queryForm.username,this.queryForm.name,this.clazzId).then(res => {
        this.total = res.result.total;
        this.tableData = res.result.data
      });
    },
    handleSizeChange(pageSize){
      studentmApi.getPageSize(pageSize,this.queryForm.username,this.queryForm.name,this.clazzId).then(res=>{
        this.total = res.result.total;
        this.tableData = res.result.data
      })
    },
