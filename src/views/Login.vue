        ...
        <div class="login-form">
            ...
            <el-form ref="form" :model="formData" :rules="rules" class="form-content" label-width="0px">
                <el-form-item prop="username">
                    <el-input onkeyup="value=value.replace(/[^\d]/g,'')" placeholder="学号/工号" v-model="formData.username">
                ...
                <div class="register-button">
                    <el-button @click="Alogin()" type="success">管理员登录</el-button>
                </div>
            </el-form>
        </div>
    </div>
                ...
                this.$refs.form.validate(valid => {
                    if (valid) {
                      if (this.formData.userType === '1'){
                        studentLogin(this.formData.username,this.formData.password).then(res => {
                          if (res.code === 200){
                      ...
                      if (this.formData.userType === '2'){
                        teacherLogin(this.formData.username,this.formData.password).then( (res) => {
                          if (res.code === 200){
                        ...
                        // login(this.formData.username, md5(this.formData.password + md5(this.formData.username)), this.formData.userType).then(res => {
